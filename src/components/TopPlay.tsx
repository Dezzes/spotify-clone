...
interface TopChartCardProps {
  ...
  handlePlayClick: (song: Track, index: number) => void;
}
...
function TopPlay() {
  ...
  const { data } = shazamApi.useGetTracksQuery();
  ...
  function handlePlayClick(song: Track, index: number) {
    console.log("play");
    dispatch(setActiveSong({ track: song, tracks: topPlays, index }));
    ...
    <div className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] max-w-full flex flex-col">
      ...
      <div className="w-full flex flex-col mt-8">
        ...
        <Swiper
          ...
          className="mt-4"
        >
          {topPlays?.slice(0, 5).map((song: Track) => (
            <SwiperSlide
              key={song?.key}
